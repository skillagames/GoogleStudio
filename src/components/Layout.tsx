...
const Layout: React.FC<LayoutProps> = ({ children, showBack }) => {
  ...
  return (
    <div className="flex min-h-screen flex-col pb-20">
      {/* Header */}
      <header className="sticky top-0 z-50 flex h-16 items-center justify-between border-b border-slate-200 bg-white/80 px-4 backdrop-blur-md">
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-10 flex h-16 items-center justify-around border-t border-slate-200 bg-white px-2 pb-safe shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        {navItems.map((item) => (
          <button
            key={item.path}
